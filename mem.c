...
extern struct proc* myproc(void); // just so I can call the function in the next line
...
int sys_shmat(void){
  ...
  if(arrayMem[shmid].mem){
  	//Map the memory to the process by growing the size of the proc
  	addr = PGROUNDUP(proc->sz);
  	mappages(proc->pgdir, (char*)addr, PGSIZE, V2P(arrayMem[shmid].mem), PTE_W|PTE_U); // PTE_W=permission to write   PTE_U=permission as user
  	proc->sz += PGSIZE;
  	switchuvm(proc);
