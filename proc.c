...
userinit(void)
{
  ...
  int i=0;
  for(; i<4; i++){
    p->signals[i] = 0;      // all entries initially zero.
  }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(samePriority->state != RUNNABLE){
        samePriority = p;
      }
    }
    if(highPriority->priority > samePriority->priority){
      p = highPriority;
    } else {
      p = samePriority;
    }
...
void printHello(void){
  cprintf("Hello world from console!");
}
